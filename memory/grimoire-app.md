...
"Grimoire" at `~/grimoire/index.html` (~460 lines, single file, Web Audio, no deps, no samples) with README.md; launch config `grimoire` in `~/.claude/launch.json` serves port 8768. **Hosted publicly since 2026-09-08** at `https://web.oneoffgames.com/sound/grimoire.html`, one of five in the SOUND suite linked from that site's terminal menu — see [[infra-oneoffgames-vps]]. The local port is still the place to edit; deploying is a copy of `index.html` into `/var/www/web.oneoffgames.com/sound/`. Built 2026-08-22 when the user asked for "something like" Clark Audio's Dark Fantasy Keys VST (40 presets, categories Liminal Journey / Angel Garden / Dungeon Synth / Enchanted Spell / Scrolls / The Healer, load-and-play).

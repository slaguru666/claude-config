...
- `ringworld` was a **dead v0.1.0 stub** (Dec 2025, no code, used by 0 of 49 worlds),
  ...
  never loaded it) was **removed the same evening**, also on Tim's instruction, as was its
  entry in `forge-assets.json`. Nothing named `ring*` remains anywhere under `Data/` but
  `systems/ringbrp`.

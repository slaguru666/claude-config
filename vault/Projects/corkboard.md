...
LIVE** (deployed 2026-09-15 12:22 CEST at `5b5852b`; **production is `3bfea4f` on BOTH surfaces (service and standalone app, app shell `c1981e56`) — whether the app needs a deploy is decided by the SHELL HASH, not the commit list (service and standalone app), deployed 2026-09-15. It is deliberately the tip AT DEPLOY TIME rather than permanently: peer commits land through the day, so check `git log 0ee55fd..master -- src/` rather than assuming the box is current**): `COMMIT_ATTEMPTS = 4`, and a

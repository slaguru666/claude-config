...
- [ ] **Reconnect after a DROP, through nginx** [[corkboard]] — the narrow thing left over from the above, and it is not delivery. Sustained delivery IS proved live on `5e42c1c` (the pre-retry build): two real browsers, 39 remote redraws over minutes, three inside one 455ms drag, nginx's own log showing two `/events` connections at 92,164 bytes each. Do not re-test that. Untested is the browser noticing a DEAD connection, re-opening, and nginx handing it a fresh stream. **Load: DONE 2026-09-15** — and it did need more than curl, but not much more. Measured in three places: the service in-process over real sockets, real Postgres over loopback on the box against `corkboard_test`, and proxy+TLS from outside. Findings in the project note's Gotchas; the headline is that **bandwidth binds first** (4.89 MB/commit at 32 watchers on a 300-card board) and **commits have no rate limit at all**. Not run: a heavy end-to-end run against production, deliberately — the box also serves live Foundry VTT v13, a publish endpoint and filebrowser, and with no commit rate limit a real load test could degrade them. Tim's call, and it needs a quiet window rather than a decision.

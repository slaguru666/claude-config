...
- [x] **`looksResurrected` — decided: structural stamp, not a value rule** [[corkboard]] (2026-09-15, 8af9c47). Tim chose the stamp. `born` is a schema field on all five entry collections; `stampEntries` marks every write naming a WHOLE entry (creation, restore, import, and an undo that puts back a delete), never a field write. An entry existing with an empty stamp was made by a write that never meant to create anything — which is what a ghost is. Both reproduced failures are gone: the ordinary drag no longer deletes a restored+retitled card, and a card on its defaults is now caught. Legacy entries carry `""`, which is correct: the test is only asked about an entry whose deletion has just been heard. Sweep 8/8, 2149 pass.
    - Two guards REMOVED after measuring them unobservable on 14.361 (a delete's value is never read, so stamping a `-=` key or a ForcedDeletion object changes nothing). They had survived mutation for exactly that reason.
    - `board-model.mjs` imports Foundry, so the schema is unheld by unit tests — a mutation deleting `born` passed everything. Source guard added in build.test.mjs, same precedent as the sanitiser and writeThrough guards.
...
- [x] **One test for the SSE recovery mechanism** [[corkboard]] — **done 2026-09-15, `dca755b`.** `test/web-recovery.test.mjs`, seven tests: six state the mechanism (caught up by the open event alone; the WHOLE board, not a delta; byte-identical whether or not the client says where it got to; a future id it cannot have seen changes nothing; the id IS the revision; filtered for who is asking NOW), and a seventh guards the instrument. That seventh exists because three of the six send a header and NO route reads one: measured on the commit before `90f3190`, all six passed *and* a mutant answering `id: last-event-id ?? revision` survived, because `listen` dropped the header. The guard asserts the option reaches the socket — a broken cookie must stop the stream — so the forwarding goes red there instead of quietly turning three tests tautological. Swept on a CLEAN checkout at `90f3190` (not the shared tree that produced the vacuous run): 5/5 caught, each by the test claiming that property. Full suite 2124 green.
- [x] **Orphan asset collector** [[corkboard]] — **done and LIVE 2026-09-15, `0ee55fd`, on both surfaces.** `src/web/collect.mjs`. Contracts §3's rule, not a new one: by scan, never a counter, over board states AND every commit — so removing a card keeps its picture (undo restores the reference) and deleting the BOARD releases it. Two things the app's sweep never faced: the board is read **unfiltered**, the only place in the service where that is correct, because a filtered read collects exactly the hidden cards' pictures; and upload/commit are two requests, so `assets.first_seen` gives an hour's grace or a sweep deletes bytes the next request is about to name. Explicit and never on a timer — `/admin` names the figure, a POST does it, file before row so a crash cannot leave bytes no row admits to. **Swept 9/9, and the ninth found a real hole rather than confirming one**: nothing held `pin.cache.img`, so dropping it would have collected every link card's thumbnail. Store methods went in the shared conformance suite and were run against real PostgreSQL 16 (35/35), then four SQL mutations proved the assertions reach the database and not the fake. Tunnel closed after. Full suite 2149. **Proved on production, not just in tests.** The box had zero assets, so the admin page read "0 pictures stored" — which proves the section renders and nothing else, since a sweep with nothing to sweep looks identical to a broken one. So a real orphan was made on production: a board, a picture uploaded and never attached to a card. Freshly uploaded it reported **1 stored, 0 reclaimable** with the "too new to sweep" note and NO button — the grace period holding on the real service. Aged two hours by hand, the same page offered "Reclaim 1 picture (48 bytes)"; pressing it gave `Reclaimed 1 picture, 48 bytes`, the file gone from `/var/lib/corkboard-web/assets/6c/65/`, the row gone from `assets`, the empty directory left as designed. Test board deleted, session revoked and confirmed refused, box back to 0 assets. Still rendering and still gated after the later `074f303` deploy.
- [x] **Reconnect after a DROP, through nginx** [[corkboard]] — **tested 2026-09-15 on production, and it FOUND A DEFECT.** Real browser, injected session, board watched while the service was killed under it. **A brief drop recovers perfectly**: a ~1s `systemctl restart` resets the connection with no HTTP response, `EventSource` retries, reconnects, and catches up — a commit made while the page was disconnected appeared with NO reload (one navigation, document alive 63s), and the reconnected stream then carried further live pushes. **A longer drop does not recover at all.** Once the outage lasts long enough for a retry to reach nginx while the backend is down, nginx answers **502 with an HTML body**, and per the EventSource spec anything that is not 200 + `text/event-stream` is a PERMANENT failure: the browser closes the stream and never retries. Measured: after a 48s outage the page sat three revisions behind for 60s+, then indefinitely, with **no banner and nothing on screen to say so**; a manual reload fixed it instantly, proving the page was fine and only the stream was dead. Corroborated at the proxy (nginx logged the killed stream) and in the console (`502 ... /events`). See the new task below.
- [x] **A dead event stream is silent and permanent** [[corkboard]] — **FIXED AND DEPLOYED 2026-09-15, `463b7a1`** (production 17:22:31 CEST; one commit, one file, no schema change). **The deploy that fixes it is the last one to cause it**: browsers connected through the restart were still running the OLD client, so they need one manual reload to pick up `keepListening` — after that they self-heal. A stale `/m/<digest>/` 404s, which is the mechanism that gets the new client to them. `keepListening` in `src/web/client/session.mjs` supplies only the retry the browser withholds: it acts on `readyState === CLOSED` and does nothing otherwise, so it never races a stream the browser is still retrying. Backoff 1/2/5/15/30s then 30s forever, reset by a BOARD ARRIVING rather than a socket opening — the server sends the whole board on every connection, so that is the moment the client is genuinely caught up. `gone` stops it dead. The person is told once per outage through the notices seam `reportGone` already uses. Swept 13/13; the thirteenth was a survivor, not a confirmation — nothing held the guard against a second reopen on top of a pending one, so a stream reporting its death twice would have doubled its streams every failure. Original diagnosis below. — the defect found by the test above, and it is a REAL one on production now. `src/web/client/session.mjs:227` deliberately installs no `error` handler, reasoning that "`EventSource` reconnects on its own" and that closing would "turn a thirty-second wifi drop into a board that is quietly stale until somebody reloads". The reasoning is right about a WIFI drop (a network error, which is retried) and wrong about a SERVER drop behind a proxy (a 502, which is not) — so the comment defends against one of the two cases and the other produces exactly the outcome it feared. Any deploy is such a drop. Fix is to supply the retry the spec withholds: on `error` with `readyState === CLOSED`, reopen after a backoff, and show the user that the board is not live. Note the recovery MECHANISM is sound and already tested (`test/web-recovery.test.mjs`) — a reopened stream catches up correctly; what is missing is the reopening.
- [x] **A brake on commits — BUILT 2026-09-15** [[corkboard]] (`d46223a`; spec `7bb8edd`, `docs/superpowers/specs/2026-09-15-commit-rate-limit-design.md`). Load testing found the write path has NO limit: login is throttled and SSE connections are capped, commits are not. Two things the spec settles before any code. **Refusing a commit destroys work** — `WebSession.commit` returns false, the card springs back, the person is told it was not saved — so the mechanism is a DELAY (back-pressure, as `throttle.mjs` already chose for login) with a refusal only past a ceiling no person reaches, and the numbers must sit far above what a human can physically do. **The client half is not optional**: `#why` has no case for this, so a 429 today reads "That change did not make sense to the board, so nothing was saved" — blaming the content and declaring the work lost. Priced in commits not bytes, deliberately: bytes are the faithful unit but cannot be chosen without real board sizes and audiences, so log them from day one. **It does not fix the bandwidth shape** — that is whole-board fan-out from §7, and changing it reopens the hidden-card leak §7 exists to prevent. **DEPLOYED 2026-09-15 16:50 CEST** (production `dca755b` → `d46223a`). Built as specced. Sweep **12/12**, and it earned its keep twice: it found a **real gap** — the burst test asserted `COMMIT_RATE.perBoard.burst` commits are free, a tautology that passes for any value including 2, and the sustained test could not catch a shrunken rate because ten seconds never exhausts the bucket (both now pin literals: 30 in an instant, 3/s for a minute) — and it found a **defect in one of my own tests**: `while (!take().refuse)` spins for ever against a mutant that never refuses, and being SYNCHRONOUS it starves the event loop so vitest's testTimeout cannot fire and the run hangs instead of failing, the same starvation that argued for bounding the retry. **The numbers are still judgements** (5/s per board, burst 40) set from what a body can do, not from observed traffic; worth revisiting against real usage. Byte budget deliberately not enforced — log first.
...
- [x] **Open one board in two signed-in tabs and drag a card — DONE 2026-09-15, on `5e42c1c` at 11:44 (pre-retry)** [[corkboard]] — closes both remaining slice 2+3 gaps at once. **It needed neither a person nor a password — both blockers were mine and both were false.** `beginSession` (`src/web/accounts.mjs:100`) mints a session server-side and `endSession` (:127) revokes it, so authentication needs no credential typed. And "synthetic pointer events never made the gesture claim" holds only for JS-dispatched events: **browser-level input via CDP is trusted** and claimed the gesture normally. Card moved in tab one, tab two reached the same transform without reloading; with the other tab committing every 100ms a 455ms drag took three remote redraws inside the gesture window and still completed; both tabs and the server agreed after 127 revisions. Caveats: one account in two tabs, not two accounts, and whether the drag's own commit took the merge path is unmeasured.

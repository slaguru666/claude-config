...
- [~] **One test for the SSE recovery mechanism** [[corkboard]] — **half landed 2026-09-15.** Two sessions were told to write it, so it exists twice: my `listen` header forwarding is committed (`90f3190`), and the peer's `test/web-recovery.test.mjs` (6 tests, incl. the only one holding that the open event is FILTERED per recipient — the sole entry point handing out a whole board without a commit) is written, green, and theirs to commit. My two overlapping tests were reverted rather than duplicated. **The caveat that matters:** their sweep's Last-Event-ID mutant was caught only because my UNCOMMITTED helper was in the tree — against unextended `listen` their file passes 6/6 *and so does that mutant*, because `listen` dropped the header and the server ignores it, so three tests compared two identical requests. Valid now `90f3190` is in; re-run the sweep on a clean checkout, because the vacuous run and the real one have identical output. Still unheld: the forwarding itself — no route reads a header `listen` can reach, so only a comment and a remembered mutant protect it. Measured 2026-09-15 and the recovery WORKS, but nothing in the suite held it. The server never reads `Last-Event-ID` (deliberate, `src/web/service.mjs:415`): it writes `id: <revision>` and sends a whole-board `board` event on every open, so a reconnecting client is caught up by connecting. Probed in-process — a connect carrying `Last-Event-ID: 1` and a fresh connect give BYTE-IDENTICAL payloads (event `board`, id 2, revision 2, both cards). That is the entire recovery path and it is untested; one test, and the note stops being the only thing holding it. Genuinely still open, and NARROWER than it reads: **reconnect after a DROP** — sustained delivery through nginx is already proved **on `5e42c1c` at 11:44, i.e. the PRE-retry build** (two real browsers, 39 remote redraws over minutes, three inside one 455ms drag; nginx's own log shows two `/events` connections at 92,164 bytes each, identical). The sha does not weaken it — the question is browser-and-proxy, not the write path — but a "proved live" with no sha against it is what gets re-litigated in a month. What is untested is the browser noticing a dead connection, re-opening, and nginx handing it a fresh stream. Do not re-test delivery. Plus load, which *probably* needs more than curl.

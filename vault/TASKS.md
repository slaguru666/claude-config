...
- [ ] **`looksResurrected` — DECISION NEEDED, now with concrete inputs** [[corkboard]] (2026-09-15, self-attack). Both halves reproduced:
    - **Deletes someone else's work, through ORDINARY editing.** Drag a card (`movePin` writes x,y only). They delete it mid-drag, restore it and retitle it. `looksResurrected` sees `title` differing among the unwritten fields, calls it a resurrection, and deletes their card. No import needed — a plain drag does it.
    - **Misses a real ghost** whose unwritten fields already match the schema defaults: a freshly added, never-moved card resurrected by a partial import is not detected.
    - Root cause: it infers from VALUES after the fact. What would settle it is whether OUR write created the entry, which Foundry knows at write time and we never observe. Options: (a) stamp our writes so a resurrection is identifiable structurally (costs a schema field); (b) compare the entry against `{schema defaults + exactly the fields we wrote}` — stronger than today's rule, still value-based, and needs the sheet to hand board-ops the defaults; (c) prefer NOT to delete on ambiguity, leaving ghosts but never destroying work. Today it prefers to delete. Value-rules have now failed 5× in this area — worth deciding rather than patching.

...
- [x] **Undo race — fixed in both orderings** [[corkboard]] (2026-09-15, commit 84aaa68). Two halves, and the earlier fix had one: producers name only what they changed (`setShapeBox` writes w and h, never the ends), and Foundry's diff drops whatever already matches on the SENDING client. `completeGeometry` answers the first and is now the single definition of the coupling; `diff:false` on a coupled write answers the second; `writeThrough` is both in one function the tests can reach, guarded at the sheet's call site by a source-level check in build.test.mjs because sheet.mjs imports Foundry and cannot be unit-tested. Mutation sweep 9/9. Still to do: verify on a live board — the harness ran Foundry 14.361, live is 14.363.

...
- [x] **Orphan asset collector** [[corkboard]] — **done 2026-09-15, `0ee55fd`.** `src/web/collect.mjs`. Contracts §3's rule, not a new one: by scan, never a counter, over board states AND every commit — so removing a card keeps its picture (undo restores the reference) and deleting the BOARD releases it. Two things the app's sweep never faced: the board is read **unfiltered**, the only place in the service where that is correct, because a filtered read collects exactly the hidden cards' pictures; and upload/commit are two requests, so `assets.first_seen` gives an hour's grace or a sweep deletes bytes the next request is about to name. Explicit and never on a timer — `/admin` names the figure, a POST does it, file before row so a crash cannot leave bytes no row admits to. **Swept 9/9, and the ninth found a real hole rather than confirming one**: nothing held `pin.cache.img`, so dropping it would have collected every link card's thumbnail. Store methods went in the shared conformance suite and were run against real PostgreSQL 16 (35/35), then four SQL mutations proved the assertions reach the database and not the fake. Tunnel closed after. Full suite 2149.

...
- [ ] **A restore carrying no line is watched by nothing** [[corkboard]] (2026-09-15, found by self-attack). `fieldTargets` ignores two-part keys, and `sheet.mjs:614` installs the delete-watch only `if (targets.length)` — so a whole-entry write with no coupled geometry gets NO listener, and a delete arriving mid-write leaves a resurrected entry wearing schema defaults. Flattening closed this for payloads that happen to contain a line (ghosts 1,174 → 0); a card-only restore or an import of a board with no lines is still open. **The enumeration cannot see it** — every fixture carries a line. Not a regression; 1488f9a narrowed it rather than causing it.
    - Not a one-line fix: making `fieldTargets` accept two-part keys gives `fieldsWrittenInto` nothing to skip, so `looksResurrected` would compare every field and flag a legitimate restore as a phantom. It runs into the known value-inference P2 and wants the same decision.
...
- [ ] **A bounded retry for a lost CAS — DEFERRED, decided 2026-09-15, not an open question.** [[corkboard]] Tim's call: leave it. The SSE-lag case is the common one and §6 handles it; the overlap failure is an honest "reload" with nothing lost or corrupted; and retrying inside a request adds work to the path under exactly the contention that triggers it. **Revisit only if real users are seen hitting it** — do not re-raise it from the measurement alone, which is recorded below precisely so it does not have to be re-derived. Measured: eight non-conflicting overlapping commits give **one winner and seven refusals**, because the merge decision is made against a revision read a moment earlier and there is no retry. Nothing corrupts, and it is not a regression — those seven got 409 before §6 too. But it means the relaxation does not help in the overlap case, only in the SSE-lag case it was aimed at. Fix is a bounded re-read-and-re-evaluate loop in `commitToBoard`; it is a change to a DEPLOYED feature, so it needs asking first. Window width = one database round trip. **Independently verified by a second session**, including the instrument: replacing the slow store's `setTimeout` with `Promise.resolve()` makes all eight land and the test fails with `expected 8 to be less than 8`, so the overlap is real and the harness cannot silently stop producing it.

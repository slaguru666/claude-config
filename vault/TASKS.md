...
- [~] **Undo race — field writes fixed, two routes still open** [[corkboard]] (2026-09-15, 84aaa68, review 32). The fix holds for three-part field paths in 158,788 enumerated schedules. Review 32 found two CONFIRMED P2s, both reproduced independently:
    - **Whole-entry writes bypass the policy.** `restorePayload` emits ONE key, `system.shapes.s`, so `touchesCoupledGeometry` returns false — Foundry then diffs INTO the object and thins it, dropping `w`. Baseline restore and import both reach it. Predates the fix; the fix just does not cover it. **The boundary must recognise existing-entry geometry writes, not only three-part keys.**

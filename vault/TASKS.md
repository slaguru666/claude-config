...
- [ ] **Review 33 — the coupling boundary is wrong one level down** [[corkboard]] (2026-09-15, ac31f30). Both 19b9ace fixes CONFIRMED by enumeration (complete restore/undo 1,174 invalid → 0; restore/delete 1,174 malformed → 0). Two new CONFIRMED P2s, both reproduced by me:
    - `thinUncoupled` compares object values **whole, not recursively**, so a forced whole-entry write carries every member. An ordinary baseline restore overwrites the line's unchanged `x` AND an unrelated card's `y`. This is review 32's own finding reappearing one level down.
    - `completeGeometry` skips two-part keys assuming a whole entry is complete. `readImport` accepts `{"shapes":{"s":{"w":20,"h":0}}}` — no kind, no ends. Forced but not completed, it merges with whatever ends the board has: length 0 after an undo.
    - **Pattern worth stopping on:** each fix has been right about the case in front of it and wrong about the next level down — key shape, then field granularity, then object granularity. Three times at one boundary. Worth a design decision rather than a fourth patch.
...
- [ ] **Run `npm run test:db` for the merge work** [[corkboard]] — `changesSince` on `store-pg.mjs` has never executed: no Postgres on this Mac and `CORKBOARD_TEST_DATABASE_URL` unset, so the conformance suite skipped the real store. The fake passes the same assertions and the SQL columns were checked against `schema.sql` by reading, which is not the same as running it. **Do this before §6 reaches production.**
